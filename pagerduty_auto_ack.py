...
USER_AGENT = "pagerduty-local-auto-ack/1.0"
...
def build_config(argv: list[str]) -> tuple[Config, bool]:
    ...
    parser.add_argument("--once", action="store_true", help="Run one poll cycle and exit. This is the default.")
    parser.add_argument("--watch", action="store_true", help="Keep polling until stopped.")
    parser.add_argument("--apply", action="store_true", default=env_bool("PD_APPLY"), help="Actually acknowledge incidents.")
    parser.add_argument("--dry-run", action="store_true", help="Force dry-run mode even if PD_APPLY=true.")
    parser.add_argument("--interval", type=int, default=env_int("PD_POLL_SECONDS", 30), help="Polling interval in seconds for --watch.")
    ...
    return config, bool(args.watch)


def main(argv: list[str]) -> int:
    try:
        config, watch = build_config(argv)
